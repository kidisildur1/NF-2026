const facts=[
  ...
  ['🎬','КИРА Найтли','Все фильмы с Кирой Найтли — не просто просмотрены. Они прошли строгую экспертную оценку.'],
  ['😅','НО ЕСТЬ НЮАНС','Это не фраза. Это фирменный сигнал: дальше события станут гораздо интереснее.'],
  ['🦫','КАПИБАРА — ЕГО ТЕМА','За совет спасибо, но есть нюанс: капибара уже назначена главным советником по спокойствию.'],
...
let current=0;const $=id=>document.getElementById(id);function renderFact(){const[e,t,x]=facts[current],panel=artPanels[current],col=panel%4,row=Math.floor(panel/4);$('factEmoji').textContent=e;$('factTitle').textContent=t;$('factText').textContent=x;$('count').textContent=`ФАКТ ${current+1} / ${facts.length}`;$('progressBar').style.width=`${(current+1)/facts.length*100}%`;$('factArt').style.backgroundPosition=`${col*33.333}% ${row*33.333}%`;$('nextFact').textContent=current===facts.length-1?'ОТКРЫТЬ ПОЗДРАВЛЕНИЕ →':'СЛЕДУЮЩИЙ ФАКТ →'}function burst(n=90){const colors=['#ffdf00','#ff3cac','#00e5ff','#fff'];for(let i=0;i<n;i++){const p=document.createElement('i');p.className='piece';p.style.left=`${Math.random()*100}%`;p.style.setProperty('--x',`${Math.random()*120-60}px`);p.style.setProperty('--drift',`${Math.random()*260-130}px`);p.style.setProperty('--color',colors[i%colors.length]);$('confetti').append(p);p.addEventListener('animationend',()=>p.remove())}}$('nextFact').addEventListener('click',()=>{if(current<facts.length-1){current++;renderFact();burst(34)}else{$('factsScreen').hidden=true;$('greetingScreen').hidden=false;burst(220);window.scrollTo({top:0,behavior:'smooth'})}});$('chaos').addEventListener('click',()=>burst(250));renderFact();
